...
install_snopt_interface()
{
	...
	sudo make install
	cd include
	sudo mkdir -p /usr/include/snopt && sudo cp -rf * /usr/include/snopt
	echo "snopt cpp installed successfully"
}

install_dependencies()
{
        sudo apt-get -y install autoconf autogen
}
...
install_snopt_binary()
{
	...
	cd $SCRIPT_DIR/lib && sudo cp -rf * /usr/lib
	echo "install snopt.pc package configure file......"
	cd $SCRIPT_DIR && sudo cp -rf snopt.pc /usr/lib/pkgconfig
